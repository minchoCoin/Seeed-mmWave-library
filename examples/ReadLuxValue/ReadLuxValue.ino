...
void setup() {
    ...
    // BH1750_TO_GROUND or BH1750_TO_VCC should be used depending on how the address pin of the sensor is wired
    // This function will return false if no sensor is found or if it's already connected to I2C

    // Check if the sensor is available
    if (!avail) {
        Serial.println("No BH1750 sensor found!");  // Print an error message if the sensor is not found
        while (true) {};  // Loop indefinitely if the sensor is not found
    }
...
void loop() {
    ...
    // You can add other code here that will run concurrently with the sensor measurements
    //...
}