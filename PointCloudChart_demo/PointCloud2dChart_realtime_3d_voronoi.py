...
def voronoi_upsampling(points, speeds, bounds=None, max_points_to_add=100):
    """
    3D 보로노이 다이어그램을 사용한 포인트 클라우드 업샘플링
    ...
    try:
        # 노이즈 추가하여 중복 좌표 방지
        points_with_noise = points + np.random.normal(0, 1e-8, points.shape)
        vor = Voronoi(points_with_noise)
        
        # 보로노이 정점 필터링 (경계 내부에 있는 정점만 선택)
        valid_vertices = []
        for vertex in vor.vertices:
            if (bounds[0] <= vertex[0] <= bounds[1] and
                bounds[2] <= vertex[1] <= bounds[3] and
                bounds[4] <= vertex[2] <= bounds[5]):
                valid_vertices.append(vertex)
        
        if not valid_vertices:
            return points, speeds

        # 속도 값 계산 - 각 정점에 대해 가장 가까운 원본 포인트의 속도를 가중 평균
        new_speeds = []
        for vertex in valid_vertices:
            dists = np.sqrt(np.sum((points - vertex) ** 2, axis=1))
            k = min(3, len(points))  # 가장 가까운 k개 포인트 사용
        ...
        if len(valid_vertices) > max_points_to_add:
            # 가능한 한 균일하게 샘플링 (단순 랜덤 대신)
            # 각 정점의 isolation 계산 (다른 정점으로부터의 최소 거리)
            if len(valid_vertices) > 1:  # 정점이 충분히 있을 때만 수행
                vertex_array = np.array(valid_vertices)
                isolation_scores = []
                for i, v in enumerate(vertex_array):
                    other_vertices = np.delete(vertex_array, i, axis=0)
                    if len(other_vertices) > 0:
                        min_dist = np.min(np.sqrt(np.sum((other_vertices - v)**2, axis=1)))
                        isolation_scores.append(min_dist)
                    else:
                        isolation_scores.append(0)
                
                # isolation이 높은 순서대로 정렬
                sorted_indices = np.argsort(isolation_scores)[::-1]
                selected_indices = sorted_indices[:max_points_to_add]
            else:
                selected_indices = [0]
                
            valid_vertices = [valid_vertices[i] for i in selected_indices]
            new_speeds = [new_speeds[i] for i in selected_indices]
        ...
        print(f"Voronoi 계산 오류: {e}")
        import traceback
        traceback.print_exc()
        # 보로노이 계산 실패 시 다른 방법으로 업샘플링 시도
        return points, speeds
