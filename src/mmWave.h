...
class SeeedmmWave {
  ...
  protected:
    ...
    virtual bool handleType(uint16_t _type, const uint8_t *data, size_t data_len) = 0;
...
class mmWaveBreath : public SeeedmmWave {
  private:
    ...
    HeartBreathDistance* _heartBreathDistance; // Raw pointer for HeartBreathDistance

  public:
    mmWaveBreath()
        : _heartBreath(nullptr), _breathRate(nullptr), _heartRate(nullptr), _heartBreathDistance(nullptr) {}
    ...
    bool handleType(uint16_t _type, const uint8_t *data, size_t data_len) override {
        ...
        delete _heartBreath; _heartBreath = nullptr;
        delete _breathRate; _breathRate = nullptr;
        delete _heartRate; _heartRate = nullptr;
        delete _heartBreathDistance; _heartBreathDistance = nullptr;

        TypeHeartBreath type = static_cast<TypeHeartBreath>(_type);
        switch (type) {
            case TypeHeartBreath::TypeHeartBreathPhase:
                _heartBreath = new HeartBreath(
                    extractFloat(data),
                    extractFloat(data + sizeof(float)),
                    extractFloat(data + 2 * sizeof(float)));
                break;
...
class mmWaveFall : public SeeedmmWave {
  ...
  public:
     ...
     */
    bool getRadarParameters(float &height, float &threshold, uint32_t &sensitivity) {
        getRadarParameters();
        if (_parametersValid) {
            height           = _height;
    ...
    bool getRadarParameters(float &height, float &threshold, uint32_t &sensitivity, float &rect_XL, float &rect_XR, float &rect_ZF, float &rect_ZB) {
        getRadarParameters();
        if (_parametersValid) {
            height           = _height;
            threshold        = _thershold;
    ...
    bool setSensitivity(uint32_t _sensitivity) {  // Range: 3~10 average of 3 frames of data?
     ...
     */
    bool setAlamAreaParameters(float rect_XL, float rect_XR, float rect_ZF, float rect_ZB) {
    ...
    bool get3DPointCloud(int option)  // option 0: detection results, 1: target information;
    {
        uint16_t type = option == 0 ? static_cast<uint16_t>(TypeFallDetection::Report3DPointCloudDetection) : static_cast<uint16_t>(TypeFallDetection::Report3DPointCloudTartgetInfo);
        if (this->SendFrame(type, nullptr, 0)) {
            return fetch(type, 1000);  // Timeout of 1000ms to fetch the response
        }
    }
